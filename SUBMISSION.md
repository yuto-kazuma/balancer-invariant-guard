...
**Subject:** Take-Home Submission - Balancer V2 Exploit Analysis and InvariantBreaker PoC
...
Unlike key-compromise or bridge infra attacks, this is pure smart-contract math: deterministic, reproducible, and preventable with invariant checks and executable PoCs.
...
- `poc/`: minimal vulnerable pool and Foundry exploit reproducing D collapse and attacker profit
- `tool/rounding_scanner.py`: working analyzer that flags the exact Balancer pattern
- `demo.sh`: one command runs scan and PoC
